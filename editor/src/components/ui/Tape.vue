<template>
	<div class="container">
		...
		<!-- 
		<button @click="changeArray('left', 1)" class="test-btn">
			Move Left
		</button>
		<button @click="changeArray('right', 2)" class="test-btn">
			Move Right
		</button> -->
	</div>
	<div class="flex" v-if="showButtons">
		...
		<button class="btn mx-5" @click="nextStepHandler">
			Next
			<span>
				<svg
					xmlns="http://www.w3.org/2000/svg"
		...
		<button class="btn mr-auto" @click="playHandler" v-else>
			<span>
				<svg
					xmlns="http://www.w3.org/2000/svg"
					class="h-6 w-7"
...
.btn {
	@apply flex flex-col bg-cyan-300 w-16 h-14 p-2 text-gray-800 text-xs rounded-2xl font-medium shadow-lg shadow-cyan-500/50 outline-none hover:translate-z-10;
}
</style>
